...
1. If any of the ncurses functionality is to be used in this program, then compile with "gcc -o PXRW PXRW.c -lncurses".
...
4) If i2c-tools has been installed on the computer which is running this code, there may be an i2c-dev.h file located at
   /usr/local/include/linux/i2c-dev.h. If so, this file should be deleted as it will cause compilation errors for this program.
   Deletion of this file should not affect the terminal commands (e.g. i2cset, i2cget, i2cdetect) created i2c-tools package.
...
#include <linux/i2c.h>
//#include "i2c-dev.h" // for i2c_smbus_* functions
#include <stdio.h>
#include <sys/ioctl.h>
//#include <errno.h>
...
int main(int argc, char **argv)
{
    ...
    /* the following code is not necessary if the ioctl(file, I2C_RDWR, &packets) command is to be used */
    /*
    ...
    */
    ...
    //readOnceFrame(file);
    //readOnceIntegralFrame(file);
    autoReadFrame(file);
    //autoReadIntegralFrame(file);

    //NOTE: BLOCK READ DOESN'T SEEM TO WORK WITH THE PX4FLOW
    //read_1 = i2c_smbus_read_i2c_block_data(file, reg_r1, 5, test_array);
